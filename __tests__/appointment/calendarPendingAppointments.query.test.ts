...
describe('calendarPendingAppointments Query Resolver', () => {
    ...
    it('should return pending appointments for a patient', async () => {
        ...
        const result = await queries.Query.calendarPendingAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 1 }, context);
    ...
    it('should return pending appointments for a doctor', async () => {
        ...
        const reservedTimes = [{ appointment_start: new Date(Date.now() + 100000) }];
        const mockAppointments = [
    ...
    it('should return pending appointments for an admin', async () => {
        ...
        const result = await queries.Query.calendarPendingAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 2 }, context);
    ...
    it('should throw an error if user is not authenticated', async () => {
        ...
        await expect(queries.Query.calendarPendingAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 1 }, context))
            .rejects
            .toThrow('Action unauthorized');
    });

    it('should handle errors when fetching pending appointments', async () => {
        mockUserRepo.findOneBy.mockReturnValue({id: 1, userRoleId: 3})
       mockAppointmentRepo.createQueryBuilder.mockReturnValue({
            createQueryBuilder: jest.fn().mockReturnThis(),
            where: jest.fn().mockReturnThis(),
        ...
        await expect(queries.Query.calendarPendingAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 1 }, context))
            .rejects
            .toThrow('Error fetching pending appointments: Error: Database error');
    });
});
