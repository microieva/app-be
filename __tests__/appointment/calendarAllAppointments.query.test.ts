...
describe('calendarAllAppointments Query Resolver', () => {
    ...
    it('should return appointments for a patient', async () => {
        ...
        const result = await queries.Query.calendarAllAppointments(null, { monthStart: new Date(Date.now() - 300000), monthEnd: new Date(), patientId: 1 }, context);
    ...
    it('should return appointments for a doctor', async () => {
        ...
        const result = await queries.Query.calendarAllAppointments(null, { monthStart: new Date(Date.now() - 500000), monthEnd: new Date(), patientId: 1 }, context);

        expect(result.monthSlice).toEqual(mockAppointments);
    });

    it('should return appointments for an admin', async () => {
        const mockAppointments = [
        ...
        const result = await queries.Query.calendarAllAppointments(null, { monthStart: new Date(Date.now() - 800000), monthEnd: new Date(), patientId: 2 }, context);
    ...
    it('should throw an error if user is not authenticated', async () => {
        ...
        await expect(queries.Query.calendarAllAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 1 }, context))
            .rejects
    ...
    it('should handle errors when fetching appointments', async () => {
        ...
        await expect(queries.Query.calendarAllAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 1 }, context))
            .rejects
            .toThrow('Error fetching all appointments: Error: Database error');
    });
});
