...
describe('calendarPastAppointments Query Resolver', () => {
    ...
    it('should return past appointments for a patient', async () => {
        ...
        const result = await queries.Query.calendarPastAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 1 }, context);
    ...
    it('should return past appointments for a doctor', async () => {
        ...
        const result = await queries.Query.calendarPastAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 2 }, context);

        expect(result.monthSlice).toEqual(mockAppointments);
    });

    it('should return past appointments for an admin', async () => {
        ...
        const result = await queries.Query.calendarPastAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 3 }, context);
    ...
    it('should throw an error if user is not authenticated', async () => {
        ...
        await expect(queries.Query.calendarPastAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 1 }, context))
            .rejects
            .toThrow('Unauthorized action');
    });

    it('should handle errors when fetching past appointments', async () => {
        mockUserRepo.findOneBy.mockReturnValue({id: 1, userRoleId: 3});
        mockAppointmentRepo.createQueryBuilder.mockReturnValue({
            createQueryBuilder: jest.fn().mockReturnThis(),
            where: jest.fn().mockReturnThis(),
        ...
        await expect(queries.Query.calendarPastAppointments(null, { monthStart: new Date(), monthEnd: new Date(), patientId: 1 }, context))
            .rejects
            .toThrow('Error fetching past appointments: Error: Database error');
    });
});
